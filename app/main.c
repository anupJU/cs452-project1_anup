...
void print_jobs() {
    ...
    // Collect job listings into an array
    ...
    // Rearrange the array based on job number
    ...
    // Display the jobs in ascending order based on their job number
    for (int i = 0; i < job_count; i++) {
        job = job_array[i];
        if (job->done) {
    ...
    // Eliminate and release resources for completed jobs
    ...
    // Release the job array from memory
    free(job_array);
}
    ...
    continue;
        ...
        if (cmd[0] && !do_builtin(&sh, cmd)) {
            ...
            if (pid == 0) {
                ...
                // Reset signals to their default settings in the child process
                ...
                if (background) {
                    ...
                    // Await the completion of the child process
                    int status;
                    waitpid(pid, &status, WUNTRACED);

                    // Re-establish the shell as the foreground process group
        ...
        // Release the allocated memory for command line or cmd
